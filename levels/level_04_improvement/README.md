...
この Level では、Iterator の細かい仕組みよりも「検索する処理」と「表示する処理」を分けることを優先します。Iterator は値を順番に取り出す仕組みです。`Path` / `PathBuf` は、文字列ではなくファイルパス専用の型だと考えてください。
`panic` は Level 2 で見た「失敗時にプログラムを止める」動きです。この Level では、検索結果 0 件や一部ファイルの失敗を何でも panic にしない設計を学びます。
...
A. 小さなテキストでは十分です。巨大ファイルやバイナリ混在では、`BufRead` のように少しずつ読む設計を検討します。`BufRead` は発展なので、今は「大きい入力では一度に全部読まない選択肢がある」と理解できれば十分です。
```
