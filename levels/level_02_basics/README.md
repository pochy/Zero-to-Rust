...
ここで初めて、失敗を表す Rust の型を本格的に扱います。`Result` や `?` は暗記する対象ではなく、「失敗をどこへ渡すか」をコードに書く道具です。
...
`unwrap()` と `expect()` もこの Level で名前だけ出てきます。どちらも「成功値を強制的に取り出し、失敗したらプログラムを止める」操作です。回復できる失敗では、まず `Result` を使う方針にします。
